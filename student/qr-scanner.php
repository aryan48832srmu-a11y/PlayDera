...
$myActiveLoans = array_filter($loans, fn($l) => $l['student_id'] === $userId && $l['status'] === 'active');
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body class="text-white">
    <div class="flex min-h-screen">
        <aside class="w-64 glass fixed h-full p-4">
            ...
            <nav class="space-y-2">
                ...
                <a href="myloans.php" class="sidebar-link flex items-center gap-3 px-4 py-3 rounded-xl"><i data-lucide="clock" class="w-5 h-5"></i> My Loans</a>
        ...
        <main class="flex-1 ml-64 p-8">
            <header class="mb-8">
                ...
                <p class="text-gray-400">Scan QR code to issue equipment</p>
            </header>
            ...
            <div class="glass rounded-2xl p-6 mb-6">
                ...
                <p id="cameraStatus" class="text-center text-sm mt-3 text-gray-400">Click "Start Camera" to scan QR code</p>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="glass rounded-2xl p-6">
                    <h3 class="text-lg font-semibold mb-4">Upload QR Image</h3>
                    <input type="file" id="qrImage" accept="image/*" onchange="scanImage(this)" class="w-full px-4 py-3 rounded-xl">
                </div>

                <div class="glass rounded-2xl p-6">
                    <h3 class="text-lg font-semibold mb-4">Enter Equipment ID</h3>
            ...
            <div id="eqResult" class="hidden glass rounded-2xl p-6 mt-6">
                ...
                <form method="POST" action="../api/issue_item.php" class="mt-4">
                    <input type="hidden" name="equipment_id" id="eqIdVal">
                    <input type="hidden" name="student_id" value="<?= $userId ?>">
                    <button type="submit" class="w-full py-4 rounded-xl bg-blue-500 text-white font-bold">Issue Equipment</button>
                </form>
            </div>
            ...
            <div class="glass rounded-2xl p-6 mt-6">
                ...
                <div class="space-y-2 max-h-64 overflow-y-auto">
                    ...
                    <div class="flex items-center justify-between p-3 bg-gray-800/30 rounded-xl">
                        <div><p class="font-medium"><?= htmlspecialchars($eq['name']) ?></p><p class="text-gray-400 text-sm"><?= htmlspecialchars($eq['id']) ?></p></div>
                        <?php if ($eq['available_quantity'] > 0): ?>
                        <form method="POST" action="../api/issue_item.php">
                            <input type="hidden" name="equipment_id" value="<?= $eq['id'] ?>">
                            <input type="hidden" name="student_id" value="<?= $userId ?>">
                            <button type="submit" class="px-4 py-2 rounded bg-blue-500/20 text-blue-400">Issue</button>
                        </form>
                        <?php else: ?><span class="text-red-400">Out</span><?php endif; ?>
                    </div>
                    <?php endforeach; ?>
                </div>
            </div>
            ...
            <div class="glass rounded-2xl p-6 mt-6">
                <h3 class="text-lg font-semibold mb-4">My Issued Items</h3>
                <?php foreach ($myActiveLoans as $loan): ?>
                <div class="flex items-center justify-between p-3 bg-gray-800/30 rounded-xl mb-2">
    ...
    <script>
        ...
        function startCamera() {
            ...
            status.innerHTML = '<span class="text-yellow-400">Requesting camera...</span>';
            lucide.createIcons();
        ...
        function tryCamera(options, index, status, btn) {
            if (index >= options.length) {
                status.innerHTML = '<span class="text-red-400">Camera blocked! Allow in browser settings or use manual entry.</span>';
            ...
            navigator.mediaDevices.getUserMedia(options[index])
                .then(s => {
                    ...
                    btn.innerHTML = '<i data-lucide="camera"></i> Stop';
                    btn.className = 'px-4 py-2 rounded-lg bg-red-500 text-white flex items-center gap-2';
                    lucide.createIcons();
                    scanLoop();
                })
                .catch(e => { tryCamera(options, index + 1, status, btn); });
        }
